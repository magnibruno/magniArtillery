...
test('Guided Learning', async ({ page }) => {
  ...
  async function completeSection(page: any, sectionName: string): Promise<void> {
  ...
  try 
  {
    ...
    // we need to wait for the page to load after login befire continuing
    await page.waitForLoadState('networkidle');
